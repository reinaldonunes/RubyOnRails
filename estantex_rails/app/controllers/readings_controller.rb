class ReadingsController < ApplicationController
  ...
  before_action :set_books_list, only: %i[ new create edit update]
  before_action :set_goals_list, only: %i[ new create edit update]
  ...
  private
    def set_books_list
      @books_list = Book.all.collect { |b| [b.title, b.id] }
    end

    def set_goals_list
      @goals_list = Goal.all.collect { |g| [g.name, g.id] }
    end
    ...
    def reading_params
      params.require(:reading).permit(:goal_id, :book_id, :month, :progress, :evaluation)
    end
end
